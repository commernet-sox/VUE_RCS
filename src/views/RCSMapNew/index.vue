...
import json1 from "@/assets/RCSNew.json";
import {
  get,
  drawAgvMap1,
  arcImg,
  ...
  methods: {
    ...
    webSocket() {
      ...
      if ("WebSocket" in window) {
        ...
        _this.ws = new WebSocket("ws://10.27.9.154:18080/websocket"); //创建WebSocket连接
          ...
          switch (data.Action) {
            ...
            case "agv_map":
              if (data.errCode == 0) {
                ...
                _this.mapInfo="地图大小("+data.params.Result.gridRows+","+data.params.Result.gridColumns+")";
                _this.initCanvas();
              ...
              if (data.errCode == 0) {
                if (_this.carList != null) {
                  ...
                  _this.carList.forEach(element => {
                    ...
                    {
                      if(element.path==undefined || element.path==null || element.path==[])
                      {
                        ...
                        {
                          element.distance.push(Math.abs(Number(endpoint[1])-Number(startpoint[1])));
                        }
                        else
                        {
                          element.distance.push(Math.abs(Number(endpoint[0])-Number(startpoint[0])));
                        }
                        element.i=0;
    ...
    IconOut() {
      this.mapInfo = "地图大小("+this.flowData.gridRows+","+this.flowData.gridColumns+")";
    },
    carSearch(){
      var _this=this;
      _this.carShow=[];
